...
const faqs = [
  ...
  },
  {
    ...
    answer:
      "Calldata-level policies evaluate what the agent claims it will do. Two calls with identical bytes can produce different outcomes. noyeet evaluates the outcome: the invariant is a revert condition inside the transaction itself, so the preflight verdict and the on-chain enforcement are the same code path.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "If state moves between preflight and inclusion, the guard's assertion at inclusion reverts the transaction atomically — the whole transaction, nothing partial. The refusal names the violated invariant (NOYEET/1:INV) with the observed and required values.",
  },
  {
