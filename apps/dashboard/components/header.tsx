...
const menus = {
  products: [
    { label: "Guard contract", description: "NoYeetGuard: execute, then assert post-state", href: "/#guard" },
    { label: "Policy VM", description: "12 rules, three verdicts, zero I/O", href: "/#guard" },
    { label: "HOLD path", description: "Legal but unusual intents escalate to a human", href: "/#mechanism" },
    { label: "Verifier", description: "Receipt digests, recomputed in the browser", href: "/dashboard#verify" },
  ],
  resources: [
    { label: "README", description: "1:1 with the code, honesty table included", href: "https://github.com/Venkat5599/KP" },
    { label: "Threat model", description: "What noyeet protects against, and what it does not", href: "https://github.com/Venkat5599/KP/blob/main/docs/threat-model.md" },
    { label: "Chaos report", description: "On-chain proofs against the deployed guard", href: "https://github.com/Venkat5599/KP/blob/main/docs/chaos-report.md" },
    { label: "Checklist", description: "What is real, what is pending", href: "https://github.com/Venkat5599/KP/blob/main/docs/PRODUCTION_CHECKLIST.md" },
  ],
};
    ...
    >
      <div className="h-20 max-[850px]:h-18 flex items-center justify-between px-4 max-[850px]:px-6">
        <a href="#" className="flex items-center gap-2 ml-4 max-[850px]:ml-0">
          ...
          <span className="text-lg font-semibold text-foreground leading-0 max-[1200px]:hidden max-[850px]:inline">noyeet</span>
        </a>
        ...
        <nav className="flex items-center gap-1 max-[1200px]:gap-0 max-[850px]:hidden">
          ...
          <DesktopDropdown
            ...
            onClose={() => setActiveMenu(null)}
          />
          <a href="/execute" className="px-4 py-2 max-[1200px]:px-3 text-sm font-medium text-foreground/80 hover:text-foreground transition-colors rounded-full hover:bg-foreground/5">
            Dapp
          </a>
        </nav>

        <div className="flex items-center gap-4 max-[850px]:hidden">
          <a href="https://github.com/Venkat5599/KP" className="text-sm font-medium text-foreground/80 hover:text-foreground transition-colors">
            GitHub
          </a>
          <a href="/execute" className="group relative inline-flex items-center">
            <span className="absolute right-0 inset-y-0 w-[calc(100%-1.5rem)] rounded-xl bg-accent" />
            <span className="relative z-10 px-5 py-3 rounded-xl bg-foreground text-background text-sm font-medium">Open dapp</span>
          ...
          >
            <div className="px-6 pb-4">
              <nav className="space-y-0">
                <a href="/execute" className="flex items-center justify-between py-4 text-base font-medium text-foreground border-b border-foreground/10" onClick={closeMobile}>
                  Dapp
                </a>
                <MobileExpandable
                  label="Products"
                ...
                <MobileExpandable
                  ...
                  onClose={closeMobile}
                />
                <a href="/execute" className="flex items-center justify-between py-4 text-base font-medium text-foreground" onClick={closeMobile}>
                  Dapp
                </a>
              </nav>
              ...
              <div className="flex items-center justify-between pt-8 pb-2">
                ...
                <a href="/execute" className="group relative inline-flex items-center mt-2">
                  <span className="absolute right-0 inset-y-0 w-[calc(100%-1.5rem)] rounded-2xl bg-accent" />
                  <span className="relative z-10 px-5 py-3 rounded-2xl bg-foreground text-background text-sm font-medium">Open dapp</span>
                  <span className="relative -left-px z-10 w-10 h-10 rounded-2xl flex items-center justify-center text-black">
                    <ArrowDownRight className="w-4 h-4 transition-transform duration-300 group-hover:-rotate-45" />
                  </span>
                </a>
              </div>
