...
| `apps/dashboard` | Next.js | Dapp shell: `/` execute (policy → simulate → broadcast; value ≥ hold threshold escalates to HOLD), `/policy` n8n-style drag-and-drop policy canvas, `/overview`, `/guard`, `/verdicts`, `/transactions`, `/holds` (release/cancel buttons), `/verifier`, `/operations` (observability) + `/api/execute`, `/api/probe`, `/api/health`, `/api/metrics`, `/api/transactions`, `/api/holds`, `/api/position` (connected wallet's live position), `/healthz`, `/readyz` and the gateway surface `/v1/*`; wallet connect (injected only, never signs) in the top bar |
